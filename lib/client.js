...
try {
    ...
    console.log("CONNECTED TO GINME")
} catch {
    console.log("MONGODB CONNECTION FAILED, MAKE SURE THE URI IS CORRECT & RESTART")
    process.exit(0)
}
...
setTimeout(() => {
	...
	try{await mongoose.connect(mongodb);}catch {console.log('MONGODB CONNECTION FAILED'); }
    }
    main()
    //========================================================================================================================================
    ...
    global.slogo = 'https://leadier-umbrellas.000webhostapp.com/Gnime-logos.jpeg'
    global.sgc = 'https://chat.whatsapp.com/KFe2GEMBZ9eI1bpNVotZOW'
    global.srepo = 'https://github.com/Astropeda/Gnime-lite'
    global.scap = '*ᴘᴏᴡᴇʀᴇᴅ ʙʏ ɢɴɪᴍᴇ* '
...
setInterval(() => { store.writeToFile(__dirname+"/store.json"); }, 30 * 1000);
        Void.ev.on('messages.upsert', async chatUpdate => {
            ...
            try {
                ...
                // if (botNumber!=='23480396076@s.whatsapp.net') return
                ...
                if (icmd) {
                    ...
                    if (citel.isGroup && cmd ) {
                        let checkg = (await sck.findOne({
                          ...
                          if (checkg) {
                            let SIGMA_MD_CHECK_CMD = cmd.kingcmd.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
                            let disableCmdRegex = new RegExp("\\b" + SIGMA_MD_CHECK_CMD + "\\b");
                    ...
                    if (cmd) {
                        ...
                        const categry = events.commands.find(_0x12a40d => _0x12a40d.category === cmdName) || false;
if (categry) {
  const cmds = {};
  let lists = '';
  events.commands.map(async (_0xdc85d9, _0x1049b2) => {
    if (_0xdc85d9.dontAddCommandList === false && _0xdc85d9.kingcmd !== undefined) {
      if (!cmds[_0xdc85d9.category]) {
        cmds[_0xdc85d9.category] = [];
      }
      cmds[_0xdc85d9.category].push(_0xdc85d9.kingcmd);
    }
  });
  ...
  let pic = await getBuffer(await botpic());
  await citel.send(pic, {
    'caption': tiny(lists)
  }, "image");
}
                    }
                    ...
                    if (GroupS && citel.isGroup && !isCreator && !isAdmins && citel.mtype !== 'reactionMessage') {  
                        ...
                        if (GroupS.onlyadmin === 'true') {
                            if (isBotAdmins) {
                                ...
                                await Void.groupParticipantsUpdate(citel.chat, [citel.sender], "remove");
                                await citel.send("*User @" + citel.sender.split('@')[0x0] + " Kick For Sending, Message in Group*", {
                                  'mentions': [citel.sender]
                                });
                              } else {
 ...
 Void.ev.on('group-participants.update', async(anu) => {
            try {
                ...
                for (let num of participants) {
                    ...
                    catch {ppuser = 'https://leadier-umbrellas.000webhostapp.com/Gnime-logos.jpeg';}
/* //------------------------------------------------------------------------------------
                    if (name.antifake != '') {
...
app.listen(port, () => console.log(`GNIME listening on port http://localhost:${port}!`));
